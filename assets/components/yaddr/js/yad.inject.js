...
(function (window, document, $, undefined) {
    ...
    // mapping between what we want and <a> element properties
        ...
        parse = function (self, url) {
          ...
          if (!self.protocol && !/^([a-z]+:)?\/\//.test(url)) { // is IE and path is relative
            ...
            for (i = 0, props = ['protocol', 'user', 'pass', 'host', 'port'], s = props.length; i < s; i++) {
  ...
  window.yaddr = {
    titleVar: 'TITLE_VAR', bodyVar: 'BODY_VAR', keywordVar: 'KEYWORD_VAR',
    run: function () {
      var url = $.trim($('input[name^=href]').val());
      if (url !== '') {
        var _url = new Url('http://' + url);
        var paramTitle = 'ydTitle';
        var paramBody = 'ydBoby';
        var title = $('input[name^=title]').val();
        var body = $('input[name^=body]').val();

        _url.hash = '';
        _url.query[paramTitle] = title;
        _url.query[paramBody] = body;
        _url.query.utm_term = '';
